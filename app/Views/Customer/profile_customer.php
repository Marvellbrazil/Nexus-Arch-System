                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-2xl font-bold"><?= $data['stats']['total_tickets_per_week'] ?></p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <span class="px-3 py-1 bg-gray-100 text-gray-700 text-sm rounded-full">
                        <?= count($data['assigned_projects']) ?>
                        Project<?= count($data['assigned_projects']) > 1 ? 's' : '' ?>
                    </span>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <?php foreach ($data['assigned_projects'] as $project):
                        ...
                        $colorIndex = $project['project_id'] % count($projectColors);
                        $projectColor = $projectColors[$colorIndex];

                        $bgColors = ['bg-blue-200', 'bg-purple-200', 'bg-green-200', 'bg-orange-200', 'bg-pink-200'];
                        $textColors = ['text-blue-600', 'text-purple-600', 'text-green-600', 'text-orange-600', 'text-pink-600'];

                        $bgColor = $bgColors[$colorIndex];
                        $textColor = $textColors[$colorIndex];
                        ?>
                        <div class="bg-gradient-to-r <?= $projectColor ?> rounded-xl p-5 border project-card">
                            <div class="flex items-start justify-between mb-4">
                                <div>
                                    <h4 class="text-lg font-semibold text-gray-800 mb-1"><?= esc($project['project_name']) ?>
                                    </h4>
                                    <p class="text-gray-600 text-sm">
                                        <?= !empty($project['description']) ? esc($project['description']) : 'No description available' ?>
                                    </p>
                                </div>
                                <div class="w-10 h-10 <?= $bgColor ?> rounded-lg flex items-center justify-center">
                                    <i class="fas fa-project-diagram <?= $textColor ?>"></i>
                                </div>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <div class="flex items-center gap-2">
                                    <i class="fas fa-ticket-alt text-gray-500"></i>
                                    <span class="text-gray-700"><?= $project['ticket_count'] ?>
                                        ticket<?= $project['ticket_count'] != 1 ? 's' : '' ?></span>
                                </div>
                                <a href="<?= base_url('customer/project_detail/' . $project['project_id']) ?>"
                                    class="px-2 py-1 bg-white/50 text-secondary text-xs font-medium rounded hover:bg-white/80 transition-colors">
                                    View Details
                                </a>
                            </div>
                            ...
                            <div class="mt-3 grid grid-cols-3 gap-2 text-xs">
                                <div class="text-center">
                                    <div class="font-semibold text-blue-600"><?= $project['open_tickets'] ?? 0 ?></div>
                                    <div class="text-gray-600">Open</div>
                                </div>
                                <div class="text-center">
                                    <div class="font-semibold text-yellow-600"><?= $project['in_progress_tickets'] ?? 0 ?></div>
                                    <div class="text-gray-600">Progress</div>
                                </div>
                                <div class="text-center">
                                    <div class="font-semibold text-green-600"><?= $project['resolved_tickets'] ?? 0 ?></div>
